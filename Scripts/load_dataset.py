...
def load_files(files):
	...
	for file in files:		
		...
		cut_rows = int(len(temp.index) * 0.1);		
		temp = temp[:][cut_rows:len(temp.index) - cut_rows]			
		df = df.append(temp, ignore_index=True);		
	return df;

memory_df = load_files(memory_files);
relax_df = load_files(relax_files);
relax_music_df = load_files(relax_music_files)

print(memory_df.head())
print(relax_df.head())
print(relax_music_df.head())
